...
その他の面接のコツとしては、その面接で何の能力が評価されているのかを意識し、可能なら探りを入れるというものがあります。アルゴリズム取引系の採用プロセスはBig Techの面接のように構造化されていて、求められているスキルがちょっと違うという程度の話なので、面接ごとに何が見られているかを適切に理解して振る舞うことである程度の評価ハックが可能です。（Big Techの技術面接については以下の記事が非常に詳しく解説をしています：[Twitter で医師を拾ってきて Google のソフトウェアエンジニアにするだけの簡単なお仕事](https://nuc.hatenadiary.org/entry/2021/03/31)。この記事を理解・咀嚼する際には、以下も参考になります：[Twitter で医師が拾われて Google のソフトウェアエンジニアになって 3 年半が過ぎました](https://nodchip.hatenadiary.org/entry/2023/03/03/205125)・[スタートラインに立つためにはどうするのか](https://docs.google.com/presentation/d/1Ny4kmHE2FZMI0AuPxImokweGoAE73RAGivjDJg0kG80/edit)。）Quant TraderやQuant Researcher職では、確率統計まわりの算数の能力に加えて、１）CS/Mathの常識に基づいて、２）Systematicな再現性あるアプローチで、３）同僚と協力しながら、問題解決できること求められていて（※Juniorのポジションの場合）、各面接ではこれを更に細分化したスキルのうちいくつかを見ているわけです。何が求められているのかがわかれば、問題の解の事前分布を適切に設定できるので、求められている解のクラスがだいぶ狭まり、効率的に答えをみつけられるようにもなります。例えば、自分はQuant Traderの面接中に、深い思考力を評価するためのどちらかと言うとQuant Researcher向けの問題を出題されたのですが、スピード感を求められているのだと勘違いしてQuick and Dirtyなアプローチをしてしまい落とされたというような事も有りました。上記の対策をやって、どのようなタイプの問題がどのようなスキルを見るための出題されているかという土地勘がつかめるようになると、面接の突破率がだいぶ上がるのではないかと思います。
...
TODO
<!---
自分が就活をしている間はあまり意識しなかったのですが、アルゴリズム取引と一言に言ってもいくつか種類があり、その会社が得意としている取引戦略の種類によって求められるスキルや面接で聞かれる内容にも差が出るので、それについて多少言及します。取引戦略を分類する際には、frequency（取引頻度）とasset class（取り扱う金融商品の種類）を考えるとよいです。

frequencyについては、明確な定義はないものの、金融商品の平均保有期間が１秒以下の取引戦略のはhigh-frequencyで、数分〜数時間はmid-frequency、数日〜数週間はlow-frequencyと言って問題なさそう。

特にHFTに特化した会社はmarket makerと呼ばれたりする。 (high-frequency, mid-cap?)

hedge fund (mid/low-frequency, large-cap)。資産がでかいとそんなに急速に動かせない（Market impactがあるし。）

高頻度は、プログラミング能力やGame thoery的な話が重要（ゼロサムだし）

低頻度ほど、統計的なスキルが大事（Alternative dataからアルファを作る）


asset classの観点からすると

Equity（株）はMLとかEconとか？

Fixed income（債券）やDerivativeは数学力重視。Black scholes方程式でオプションの理論価格が計算できたりする。

あと、FX？これは金利とかには左右されるのか？

Commodityもあるが、これもデリバティブよりっぽい

最近はcryptもあるがもうココらへんはよくわからない…
...
ここまで、Quant Researcher/Trader職の就活のコツについて書いたのですが、これらの職種は日本におけるクオンツ（Quants）職とは違う気がする。

自分が就活をしていたときは、クオンツについて詳しくなかったのですが、上記の

自分の会社は、equity mid-low frequencyが見た範囲だと

銀行などで債権とかデリバティブとかのpricingしている印象。こちらのほうが古典的なクオンツ像で高度な数学を使う印象（確率解析、微分方程式）。

Algo trading firm：
シグナル
ポートフォリオ最適化などの執行
リスク
くらいに分類できそう。銀行とかはPricing以外に何してるかは知らない。

アルゴリズム取引は基本的に、Buy Sideという認識

Sell sideとBuy side、Sell sideだけでなく、債権やオプションを扱うアルゴリズム取引をする会社はSell sideにたくさんあり、銀行のクオンツ職はこれが多いと思う。

以下の記事の、金融工学クオンツというやつだろうか？

[クオンツになりたい人のための本15選‐金融工学クオンツに、おれはなる！！‐](https://quant-blogger.com/2019/11/13/book-for-fe/)

ぶっちゃけ金融工学の方がよっぽどムズそうと感じたがどうなんでしょう。（守秘義務は守る。）
